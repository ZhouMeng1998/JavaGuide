...
**我的总结：JDK编译你的JAVA文件生成.class文件，JAVA虚拟机，即JVM能识别这些.class文件，所以JAVA程序有跨平台运行的特性，无论JVM是装在Linux还是Windows上，它只认.class字节码文件**
**.class到机器码这一步，即解释，因为引入了JIT编译器，会把那些常用的代码和函数的机器码（01二进制）记录下来，这样就不用重复编译了**
...
**总结，JRE（Java Runntime Environment），即运行时环境，仅是用来运行JAVA程序，不可以创建JAVA程序；JRE是JDK的一个子集，JDK还带了编译器与工具，所以能创建以及编译程序**

#### （不重要）1.1.3. Oracle JDK 和 OpenJDK 的对比
...
- **JAVA不需要字符串结束符，因为JAVA中一切都是对象，字符串必定会存一个length的属性，编译器可以通过length来判断字符串什么时候结束，所以不需要结束符**
...
#### 1.1.7. import java 和 javax 有什么区别--->没有区别？
...
**我的总结：从java文件到.class文件是编译，从.class文件到机器码是解释，所以说JAVA编译与解释共存。之所以要先编译，是为了生成所有JVM虚拟机都能识别并解释的.class文件，这样一来，无论该JVM虚拟机是什么平台上，.class文件解释后的结果都是一样的，也就实现了"一次编译，随处可以运行"**
...
2. 含义上: **字符常量相当于一个整型值( ASCII 值),可以参加表达式运算; 字符串常量代表一个地址值(该字符串在内存中存放位置)----->字符串常量里放的是地址值，而不是字符串本身**
...
**Java的泛型是伪泛型，这是因为Java在编译期间，所有的泛型信息都会被擦掉，比如你写了一个List<String>，编译后都只会看到List，JVM看不到你的泛型。** 更多关于类型擦除的问题，可以查看这篇文章：[《Java泛型类型擦除以及类型擦除带来的问题》](https://www.cnblogs.com/wuqinglong/p/9456193.html) 。
...
**很简单，如果不重写hashCode，默认的hashCode方法即便是对于相同的对象，hashCode值也不相等，而相同对象的hashCode必须是相等的，所以要重写hashCode方法---->为什么一定要相等？参照HashSet的添加函数**
...
Java 基本类型的包装类的大部分都实现了**常量池技术，**即 Byte,Short,Integer,Long,Character,Boolean；**前面 4 种包装类默认创建了数值[-128，127] 的相应类型的缓存数据，**Character创建了数值在[0,127]范围的缓存数据，Boolean 直接返回True Or False。如果超出对应范围仍然会去创建新的对象。 为啥把缓存设置为[-128，127]区间？（[参见issue/461](https://github.com/Snailclimb/JavaGuide/issues/461)）性能和资源之间的权衡。
...
array 被初始化 arr 的拷贝也就是一个对象的引用，**也就是说 array 和 arr 指向的是同一个数组对象--->array相当于arr的替身**。 因此，外部对引用对象的改变会反映到所对应的对象上。
...
**值传递的。----->x与y为s1和s2的替身（引用），我交换s1与s2是无法影响x与y的，因为你传进来的是引用值，本质上还是一个参数。总之，尽管引用值可以修改对象内部变量的值，但它无法修改变量地址**

下面再总结一下 Java 中方法参数的使用情况：

- 一个方法不能修改一个基本数据类型的参数（即数值型或布尔型）。
- 一个方法可以改变一个对象参数的状态。
- **一个方法不能让对象参数引用一个新的对象。--->不同于指针可以改变指向的位置，参数引用只能指向初始对象**
...
> 这个并不是根本原因，面向过程也需要分配内存，计算内存偏移量，Java 性能差的主要原因并不是因为它是面向对象语言，**而是 Java 是半编译语言，java-.class->机器码，而c语言可以直接编译成机器码**，最终的执行代码并不是可以直接被 CPU 执行的二进制机械码。
>
...
2. 从变量在内存中的存储方式来看:如果成员变量是使用`static`修饰的，那么这个成员变量是属于类的，如果没有使用`static`修饰，这个成员变量是属于实例的。**而对象存在于堆内存，局部变量则存在于栈内存。-->不用了就pop掉**
...
主要作用是完成对类对象的初始化工作。可以执行。因为一个类即使没有声明构造方法也会有默认的不带参数的构造方法。**如果我们自己添加了类的构造方法（无论是否有参），Java 就不会再添加默认的无参数的构造方法了，所以创建对象的时候一定要写个默认构造方法，不然如果你写了个有参构造方法，系统就不会自动生成默认构造函数，这时候你创建子类就很可能会报错，因为它无法调用父类的默认构造方法（除非你显式地用super调用你父类的构造方法）**这时候，就不能直接 new 一个对象而不传递参数了，所以我们一直在不知不觉地使用构造方法，这也是为什么我们在创建对象的时候后面要加一个括号（因为要调用无参的构造方法）。如果我们重载了有参的构造方法，记得都要把无参的构造方法也写出来（无论是否用到），因为这可以帮助我们在创建对象的时候少踩坑。
...
由于**静态方法可以不通过对象进行调用，因此在静态方法里，不能调用其他非静态变量(依赖于对象实例而存在)**，也不可以访问非静态变量成员。
...
简单的来说：**`String` 类中使用 final 关键字**修饰字符数组来保存字符串，`private final char value[]`，所以**` String` 对象是不可变的。**
...
**总结：String，StringBuffer（加了同步锁）线程安全，StringBuilder线程不安全**

**性能**

每次对 `String` 类型进行改变的时候，都会生成一个新的 `String` 对象，然后将指针指向新的 `String` 对象。`StringBuffer` 每次都会对 `StringBuffer` 对象本身进行操作，而不是生成新的对象并改变对象引用。相同情况下**使用 `StringBuilder` 相比使用 `StringBuffer` 仅能获得 10%~15% 左右的性能提升，但却要冒多线程不安全的风险。**
...
- **String 中的 equals 方法是被重写过的**，因为 object 的 equals 方法是比较的对象的内存地址，而 String 的 equals 方法比较的是对象的值。
- 当创建 String 类型的对象时，虚拟机会在常量池中查找有没有已经存在的值和要创建的值相同的对象，如果有就把它赋给当前引用。**如果没有就在常量池中重新创建一个 String 对象,所以每次修改String对象都在常量池中生成了一个新的String对象。**
...
**Error（错误）:是程序无法处理的错误**，表示运行应用程序中较严重问题。大多数错误与代码编写者执行的操作无关，而表示代码运行时 JVM（Java 虚拟机）出现的问题。例如，**Java 虚拟机运行错误（Virtual MachineError），**当 JVM 不再有继续执行操作所需的内存资源时，将出现 **OutOfMemoryErro**r。这些异常发生时，Java 虚拟机（JVM）一般会选择线程终止。
...
**使用Java 7之后的 `try-with-resources` 语句改造上面的代码:------->把你需要释放资源的变量放在try后面的()中**
...
**程序**是含有指令和数据的文件，被存储在磁盘或其他的数据存储设备中，也就是说**程序是静态的代码。.exe可执行文件**
...
当线程执行 `wait()`方法之后，线程进入 **WAITING（等待）---->相当于闹钟，等待被唤醒**状态。进入等待状态的线程需要依靠其他线程的通知才能够返回到运行状态，而 **TIME_WAITING(超时等待)---->相当于自然醒** 状态相当于在等待状态的基础上增加了超时限制，比如通过 `sleep（long millis）`方法或 `wait（long millis）`方法可以将 Java 线程置于 TIMED WAITING 状态。当超时时间到达后 Java 线程将会返回到 RUNNABLE 状态。当线程调用同步方法时，在没有获取到锁的情况下，线程将会进入到 **BLOCKED（阻塞）** 状态。线程在执行 Runnable 的`run()`方法之后将会进入到 **TERMINATED（终止）** 状态。
...
回答：字符流是由 Java 虚拟机将字节转换得到的，问题就出在这个过程还算是非常耗时，并且，**如果我们不知道编码类型就很容易出现乱码问题---->即讲字节转换成字符的过程中出错**。所以， I/O 流就干脆提供了一个直接操作字符的接口，方便我们平时对字符进行流操作。**如果音频文件、图片等媒体文件用字节流比较好**，**如果涉及到字符的话使用字符流比较好。**
